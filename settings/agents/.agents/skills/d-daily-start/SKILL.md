...
`~/.dotfiles/vault/daily/GGGG-MM-WWW/YYYY-MM-DD.md` を準備し、前日予定の転記と Google カレンダーからの予定挿入をまとめて行う。
（daily は週フォルダ `GGGG-MM-WWW`（例 `2026-06-W27`。ISO週年+**その週の月曜日の月**+ISO週）配下に置く。`date -v-mon +%G-%m-W%V` で得る。月をまたぐ週でも1週=1フォルダを維持するため月曜基準にしている。Obsidian側フォーマット `GGGG-MM-[W]WW` は「その日の月」なので月境界の数日だけ意図的に不一致になる（Obsidianはエディタとしてのみ使う運用））
...
- daily 保存先: `~/.dotfiles/vault/daily/GGGG-MM-WWW/YYYY-MM-DD.md`（週フォルダ配下。例 `2026-06-W27`）
...
- Bash: `date -v-mon +"%G-%m-W%V"` で週フォルダ名を取得（例 `2026-06-W27`。その週の月曜日基準のISO週年+月+ISO週。`-v-mon` は今日が月曜ならそのまま、それ以外は直前の月曜に戻す。年末年始・月またぎも1週=1フォルダになる）
