...
- ファイル名形式: `yyyyMMdd_NN_機能名_plan.md`
...
- planファイルパス: `~/.dotfiles/vault/plans/`
- **planファイルは必ず日本語で記載する**（概要・対応状況・完了条件すべて）
- **簡潔に書く**。冗長な説明は不要。箇条書き中心で要点だけ

## やり取り履歴の追記（必須）

このスキル実行中の **AskUserQuestion / 議論で出た判断・補足** は、planファイル末尾の `## やり取り履歴` セクションの **最終行に逐次追記** する。

- 追記先: `## やり取り履歴` セクション末尾（既存記述の下に追加）
- フォーマット例:
  ```markdown
  ### YYYY-MM-DD HH:mm （d-plan-review）
  - Q: 〜（質問内容）
  - A: 〜（ユーザー回答）
  - メモ: 〜（議論で出た判断や補足）
  ```
